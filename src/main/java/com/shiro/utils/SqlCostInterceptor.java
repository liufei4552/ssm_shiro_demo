...
public class SqlCostInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		try {
			...
			List<ParameterMapping> parameterMappingList = boundSql
			...
			System.out.println(df.format(day) + " " + "SQL：[" + sql + "]执行耗时["
					+ sqlCost + "ms]");
		}
	}
		...
		try {
			if (parameterMappingList != null) {
				...
				if (isStrictMap(parameterObjectClass)) {
					...
					sql = handleMapParameter(sql, paramMap,
		...
		return parameterObjectClass.isPrimitive()
				|| (parameterObjectClass.isAssignableFrom(Byte.class)
						...
						|| parameterObjectClass
								.isAssignableFrom(Character.class) || parameterObjectClass
